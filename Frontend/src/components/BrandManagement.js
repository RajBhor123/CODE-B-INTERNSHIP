...
const BrandManagement = () => {
  ...
  const handleAddBrand = async () => {
    ...
    try {
      ...
      setError(error.response?.data?.error || "Failed to add brand.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteBrand = async (brandId) => {
    ...
    try {
      ...
      const canDeleteResponse = await axios.get(`http://localhost:8080/api/brands/can-delete/${brandId}`);
      ...
      const response = await axios.delete(`http://localhost:8080/api/brands/${brandId}`);
      ...
      {!showAddBrand && !showEditBrand && (
        <div className="dashboard-content">
          ...
          <div className="stats-card-icon">
            ...
            {/* Filter options - Updated to inline select elements */}
