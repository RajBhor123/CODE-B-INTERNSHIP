...
//       const brandZones = zones.filter(zone => 
//         zone.brand && zone.brand.brandName === newEstimate.brandName
//       );
...
//   // Handle form input changes for edit estimate
...
//         `http://localhost:8080/api/estimates/${editEstimateId}`, 
//         {
//           ...editEstimate,
...
//       await axios.delete(`http://localhost:8080/api/estimates/${estimateId}`);
...
//     if (estimate.chain) {
//       const chainBrands = brands.filter(b => 
//         b.chain && b.chain.chainId === estimate.chain.chainId
//       );
//       setFilteredBrands(chainBrands);
      
//       // Filter zones for the selected brand
//       const brandZones = zones.filter(z => 
//         z.brand && z.brand.brandName === estimate.brandName
//       );
//       setFilteredZones(brandZones);
//     }
...
//     const brandZones = zones.filter(zone => 
//       zone.brand && zone.brand.brandName === brandName
//     );
//     setFilteredZones(brandZones);
//   };
...
//       if (filterCompany) {
//         matchesCompany = estimate.chain && estimate.chain.chainId.toString() === filterCompany;
//       }
...
//                       <td>{estimate.chain ? (estimate.chain.companyName || estimate.chain.chainName) : "N/A"}</td>
...
//           <select
//             id="editGroupSelect"
//             value={selectedGroupId || groups.find(g => g.groupName === editEstimate.groupName)?.groupId}
...
//         const selectedChain = chains.find(chain => chain.chainId.toString() === chainId);
                  ...
                  getFilteredEstimates().map((estimate, index) => (
                    <tr key={estimate.estimateId}>
                      ...
                      <td className="action-buttons">
                        ...
                        >
                          Genarte <FontAwesomeIcon icon={faFileInvoice} />
                        </button>
                      </td>
                    </tr>
                  ))
            ...
            <div className="button-group">
              ...
              >
                {loading ? (
                  ...
                  <>
  <FontAwesomeIcon icon={faSave} /> Save Estimate
</>
)}
              </button>
            </div>
      ...
      {showEditEstimate && (
        <div className="estimate-form">
          ...
          <div className="form-container">
            ...
            <div className="form-row">
              <div className="form-group">
                <label htmlFor="editChainId">Company Name:</label>
                <input
                  id="editChainId"
                  type="text"
                  value={editEstimate.chain ? (editEstimate.chain.companyName || editEstimate.chain.chainName) : ""}
                  className="form-input"
                  disabled={true}
                />
              </div>
